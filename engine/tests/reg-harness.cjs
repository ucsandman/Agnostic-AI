...
// ── 10. embedding: the engine as a library inside another product ───────────

section(10, 'embedding: configure(), an injected registry, no repo files', ['engine/harness/index.cjs'], () => {
  const engine = req('engine/harness/index.cjs');

  test('10a. fingerprint: the same source captured twice gives one hash; a rules edit changes it', () => {
    const home = mkTmp('fp-home');
    buildClaudeHome(home);
    const a = engine.capture({ from: 'claude', home, outDir: mkTmp('fp-a') }).bundle;
    const b = engine.capture({ from: 'claude', home, outDir: mkTmp('fp-b') }).bundle;
    assert.strictEqual(a.manifest.fingerprint, b.manifest.fingerprint, 'two captures of one source must share a fingerprint');
    assert.strictEqual(engine.bundle.fingerprint(a), a.manifest.fingerprint, 'save() records fingerprint(bundle)');
    fs.appendFileSync(path.join(home, '.claude', 'CLAUDE.md'), '\n- One more rule.\n');
    const c = engine.capture({ from: 'claude', home, outDir: mkTmp('fp-c') }).bundle;
    assert.notStrictEqual(c.manifest.fingerprint, a.manifest.fingerprint, 'a rules edit must change the fingerprint');
  });

  test('10b. a host brand marks every generated file, owns its regions and its agy hook key, and is recognised on re-apply', () => {
    const previous = JSON.parse(JSON.stringify(engine.config));
    try {
      engine.configure({ brand: { id: 'host-product', mark: 'GENERATED by Host Product', region: 'host product' }, secretPatterns: ['(?i)hp_[a-z0-9]{20,}'], shimPath: '/opt/host/shim.cjs' });
      assert.strictEqual(common.GENERATED_MARK, 'GENERATED by Host Product', 'GENERATED_MARK must be live');
      assert(common.looksSecret('X', 'hp_' + 'a'.repeat(24)), 'the host secret pattern must apply');
      assert(!common.looksSecret('X', 'sk-' + 'x'.repeat(24)), 'the repo default patterns are replaced, not merged');

      const home = mkTmp('brand-home');
      buildClaudeHome(home);
      ...
      fs.writeFileSync(path.join(home, '.gemini', 'config', 'hooks.json'), '{}');
      const registry = engine.loadRegistry(home, {
        targets: JSON.parse(fs.readFileSync(path.join(ROOT, 'core', 'templates', 'targets.json'), 'utf8')).targets
          .filter((t) => ['claude', 'codex', 'agy'].includes(t.id))
          .map((t) => Object.assign({}, t, { preamble: `# Rules (${t.name})\n\nGENERATED by Host Product.\n` })),
      });
      const port = Object.assign(portPolicy(), { baseDir: ROOT });
      const { bundle: b } = engine.capture({ from: 'claude', home, outDir: mkTmp('brand-bundle'), registry, port });
      const storageDir = mkTmp('brand-storage');
      buildCodexHome(home);
      const report = engine.apply({ bundle: b, home, registry, port, storageDir, to: ['codex', 'agy'], quiet: true });
      assert.strictEqual(report.targets.codex.status, 'written', `codex must be written, got ${report.targets.codex.status}`);

      const agentsMd = fs.readFileSync(path.join(home, '.codex', 'AGENTS.md'), 'utf8');
      assert(agentsMd.includes('GENERATED by Host Product'), 'AGENTS.md carries the host mark');
      assert(!agentsMd.includes('agnostic-ai'), 'AGENTS.md must not mention the engine repo');
      const configToml = fs.readFileSync(path.join(home, '.codex', 'config.toml'), 'utf8');
      assert(configToml.includes('# >>> host product hooks start'), 'config.toml regions carry the host region name');
      assert(!configToml.includes('>>> agnostic-ai'), 'no engine-named region');
      const agyHooks = JSON.parse(fs.readFileSync(path.join(home, '.gemini', 'config', 'hooks.json'), 'utf8'));
      assert(agyHooks['host-product'], 'agy hooks.json is keyed by the host brand id');
      const chained = JSON.stringify(agyHooks);
      assert(chained.includes('/opt/host/shim.cjs'), 'ported agy hooks point at the host shim path');

      const again = engine.apply({ bundle: b, home, registry, port, storageDir, to: ['codex', 'agy'], quiet: true });
      for (const id of ['codex', 'agy']) for (const f of flattenFiles(again.targets[id])) assert(['unchanged', 'skipped-real-directory'].includes(f.action), `${id} ${f.path} must be a no-op on re-apply, got ${f.action}`);
      const st = engine.status({ bundle: b, home, registry, port, storageDir, to: ['codex', 'agy'], quiet: true });
      assert.strictEqual(st.syncState, 'synced', `status after apply must be synced, got ${st.syncState}`);
    } finally {
      engine.configure({ brand: previous.brand, secretPatterns: previous.secretPatterns, shimPath: previous.shimPath });
    }
  });

  test('10c. stripSections lives in common and sync.cjs uses the same one', () => {
    const md = '# T\n\n## Keep\n\nk\n\n## Delegation and Model Routing\n\nd\n\n## Also\n\na\n';
    const out = common.stripSections(md, ['Delegation and Model Routing']);
    assert(!out.includes('Delegation'), 'the section must be removed');
    assert(out.includes('## Keep') && out.includes('## Also'), 'neighbours survive');
    assert.strictEqual(req('engine/sync/sync.cjs').stripSections, common.stripSections, 'sync.cjs must re-export the shared function');
  });
});
