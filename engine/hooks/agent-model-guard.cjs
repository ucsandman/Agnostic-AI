...
if (toolName === "Workflow") {
  ...
  // Fan-out sizing (2026-09-19): a per-item stage over an upstream output is a
  // fan-out whose size nobody chose. harness-archaeology queued ~225 agents
  // (12 readers, 212 overlapping duplicate claims, one Sonnet skeptic each) and
  // stalled the desktop; 22 finished, none changed the decision. A script whose
  // agent() call sits inside a fan-out construct must declare its ceiling as
  // arithmetic in a comment (`// MAX_AGENTS: 24`) or a const; above FANOUT_CAP
  // it also needs `// FANOUT_OK: <why>`. Comments are masked in `masked`, so
  // the declaration is read from the original script.
  const FANOUT_CAP = 40;
  const spans = fanoutRanges(script, masked); // `ranges` above is scoped to the fable branch
  const fannedOut = calls.some((c) => spans.some(([s, e]) => c.index >= s && c.index < e));
  if (fannedOut) {
    const decl = /(?:\/\/\s*MAX_AGENTS\s*:\s*|MAX_AGENTS\s*=\s*)(\d+)/.exec(script);
    if (!decl) {
      deny(
        "BLOCKED: this Workflow script fans agent() out (parallel/pipeline/map/loop) with no declared ceiling. Write the agent count as arithmetic and put it in the script: `// MAX_AGENTS: <producers + items x verifiers + synthesizers>`; cap any term you cannot count (`items.slice(0, N)`, log the drop) and dedup items across producers before a per-item stage. Ceiling " +
          FANOUT_CAP +
          " without `// FANOUT_OK: <why>` (2026-09-19: an uncapped verify stage queued 225 agents and stalled the machine). Then re-invoke."
      );
    }
    const declared = Number(decl[1]);
    if (declared > FANOUT_CAP && !/\/\/\s*FANOUT_OK\s*:/.test(script)) {
      deny(
        "BLOCKED: this Workflow script declares MAX_AGENTS: " +
          declared +
          " (ceiling " +
          FANOUT_CAP +
          "). Either shrink the fan-out (dedup across producers, verify only what changes a decision, cap with .slice) or add `// FANOUT_OK: <why the user asked for this scale>`, then re-invoke."
      );
    }
  }
  process.exit(0);
