...
const HOOK = path.join(REPO, 'engine', 'hooks', 'gate-freeze.cjs');
const GATES = path.join(REPO, 'tools', 'gates', 'gates.cjs');
const PROJECT = 'C:/Projects/some-app';        // outside every harness root
// the pre-commit chain and settings.json live in the Claude home; hooks and gates in this repository
const HARNESS = (process.env.CLAUDE_CONFIG_DIR || path.join(require('os').homedir(), '.claude')).replace(/\\/g, '/');
const guard = REPO + '/engine/hooks/rm-guard.cjs';
...
const CASES = [
  ...
  ['relock from a project session', 'Bash', { command: 'node ' + REPO + '/tools/gates/gates.cjs --lock' }, PROJECT, 'DENY'],
  ['read a hook from a project session', 'Bash', { command: 'cat ' + guard + ' | head -20' }, PROJECT, 'ALLOW'],
  ['run a hook probe from a project session', 'Bash', { command: 'node ' + REPO + '/engine/hooks/tests/rm-guard-probe.cjs' }, PROJECT, 'ALLOW'],
  ...
  ['Edit a hook test file (not frozen)', 'Edit', { file_path: REPO + '/engine/hooks/tests/rm-guard-probe.cjs', old_string: 'a', new_string: 'b' }, PROJECT, 'ALLOW'],
...
const run = () => spawnSync('node', [GATES, 'gate-freeze'], { cwd: REPO, encoding: 'utf8' });
const lockPath = path.join(REPO, 'tools', 'gates', 'gate-manifest.lock.json');
if (!fs.existsSync(lockPath)) { console.log('FAIL  no lock file; run: node tools/gates/gates.cjs --lock'); fail++; }
  ...
  const canary = path.join(REPO, 'engine', 'hooks', 'zz-freeze-canary.cjs');   // matches hooks/*.cjs, so it is frozen but unlocked
