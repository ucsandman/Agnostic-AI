...
LOG="$CLAUDE_HOME/logs/meditation/run.log"
mkdir -p "$CLAUDE_HOME/logs/meditation"
...
CMD=("$CLAUDE_BIN" -p "/meditate $MODE" --model "$MODEL" --effort xhigh
...
TODAY="$(date +%F)"
DIGEST="$CLAUDE_HOME/meditations/digests/$TODAY.html"
LINE="$CLAUDE_HOME/meditations/digests/latest-line.txt"
