...
def _add_search_arguments(parser):
    ...
    """
    parser.add_argument("--condition", help="Condition or disease (maps to query.cond)")
    parser.add_argument(
    ...
    parser.add_argument(
        "--sort",
        help=("Sort results, e.g. 'LastUpdatePostDate:desc' or 'EnrollmentCount:asc'"),
    )
    parser.add_argument(
        "--count-total",
        dest="count_total",
...
def main():
    ...
    p_get.add_argument(
        "--fields",
        help=(
            "Comma-separated fields to return. Defaults to a useful subset if omitted."
        ),
    )
    ...
    p_search = subparsers.add_parser("search", help="Search for studies with filters")
