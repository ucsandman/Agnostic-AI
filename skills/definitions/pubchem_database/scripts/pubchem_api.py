...
def resolve(name=None, inchi=None):
    ...
    url_cids = f"{PUBCHEM_BASE_URL}/pug/compound/{input_type}/{encoded_val}/cids/JSON"
...
def range_search(feature, min_val, max_val):
    url = (
        f"{PUBCHEM_BASE_URL}/pug/compound/{feature}/range/{min_val}/{max_val}/cids/JSON"
    )
    return make_request(url)
...
def main():
    ...
    p_resolve.add_argument("--output", required=True, help="Output JSON file path")
    ...
    p_view.add_argument("--heading", required=True, help="Heading (e.g. 'Geometry')")
    ...
    p_xrefs.add_argument("--type", required=True, help="Xref type (e.g. 'PatentID')")
    p_xrefs.add_argument("--output", required=True, help="Output JSON file path")

    # Query
    p_query = subparsers.add_parser("query", help="Execute a custom PUG-REST path")
