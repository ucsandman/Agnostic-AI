...
def _check_adc() -> Mapping[str, Any]:
    try:
        ...
        cloud_rest_helpers_nodeps.get_authorized_session(skill=_SKILL, script=_SCRIPT)
    ...
    try:
        with session.request(
            ...
            json={
                ...
                "useLegacySql": False,
                "dryRun": True,
                "labels": cloud_rest_helpers_nodeps.bigquery_labels(_SKILL, _SCRIPT),
            },
            timeout=10,
        ) as response:
            response.raise_for_status()
...
def main() -> None:
    ...
    result = run_preflight(project_id=args.project_id, dataset_name=args.dataset_name)
