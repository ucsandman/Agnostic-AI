...
def _user_agent(skill: str, script: str) -> str:
    return f"{_ATTRIBUTION_PREFIX}/{_KIT_VERSION} (skill:{skill}; script:{script})"
...
class AuthorizedSession:
    ...
    def _execute_request(
        ...
        if len(stdout) < 3:
            raise CloudRestError(f"curl returned unexpectedly short output for {url}.")
    ...
    def request(
        ...
        *,
        ...
        resp = self._execute_request(method, url, body_bytes, request_headers, timeout)
