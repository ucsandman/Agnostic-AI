...
def _calculate_risk_score(telemetry: Sequence[Mapping[str, Any]]) -> str:
    ...
    bucket_risk_score_average = int(
        (bucket_risk_score_total / (len(risky_missing_fields) * (len(telemetry) or 1)))
        * 100
    )
...
def main() -> None:
    ...
    bucket_names = (
        [b.strip() for b in args.bucket_names.split(",")] if args.bucket_names else None
    )
    ...
    print(json.dumps({"risk_score": risk_score, "telemetry": telemetry}, indent=2))
