...
def read_repo(path, my_emails):
    ...
    for line in log.splitlines():
        ...
        if len(parts) == 3:
            try:
                commits.append(
                    {"at": int(parts[0]), "email": parts[1], "msg": parts[2]}
                )
            except ValueError:
                continue
    if not commits:
    ...
    mine = (
        sum(1 for c in commits if c["email"].lower() in my_emails)
        if my_emails
        else len(commits)
    )
    ...
    last_touched = sh(
        ["git", "log", "-3", "--name-only", "--format="], cwd=path
    ).lower()
    ...
    touches = [
        line_item
        for line_item in sh(
            ["git", "log", "--name-only", "--format=", "--no-merges"], cwd=path
        ).splitlines()
        if line_item.strip()
    ]
    config_touches = sum(
        1 for line_item in touches if Path(line_item).name in CONFIG_FILES
    )
...
def autopsy(repo, all_repos):
    ...
    if any(h in last for h in PAYMENT_HINTS):
        findings.append(
            (
                "payments_wall",
                "the final commits touch payment code — died at the checkout",
            )
        )
        ...
        findings.append(
            (
                "auth_wall",
                "the final commits touch auth code — died at the login screen",
            )
        )
    ...
    if (
        repo["has_readme"]
        and repo["commits"] >= 20
        and not repo["has_deploy"]
        and not repo["has_tags"]
    ):  # tagged releases = it shipped, just not as a deployment
        findings.append(
            (
                "deploy_fear",
                "README written, %d commits in, tests %s, and no deploy config anywhere. "
                "It worked. It just never shipped."
                % (repo["commits"], "present" if repo["has_tests"] else "absent"),
            )
        )
    ...
    rewrite_msgs = [
        ...
        if any(
            w in m.lower() for w in ("rewrite", "migrate to", "port to", "switch to")
        )
    ]
...
def pulse(repo):
    ...
    if repo["has_readme"]:
        score += 15
        why.append("has a README")
    if repo["has_tests"]:
        score += 20
        why.append("has tests")
    if repo["config_ratio"] < 0.5:
        score += 20
        why.append("real code outweighs config")
    score += min(15, repo["commits"] // 4)
    if repo["commits"] >= 20:
        why.append("%d commits of momentum" % repo["commits"])
    if repo["remote"]:
        score += 10
        why.append("already on a remote")
    age_days = (datetime.now().timestamp() - repo["last"]) / 86400
    if age_days < 180:
        score += 10
        why.append("corpse is still warm (<6 months)")
    if repo["has_deploy"]:
        score += 10
        why.append("deploy config already exists")
    missing = []
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Find your dead side projects and why they died."
    )
    ap.add_argument(
        "roots",
        nargs="*",
        default=None,
        help="directories to scan (default: common dev dirs)",
    )
    ap.add_argument(
        "--days", type=int, default=45, help="no commits in N days = dead (default 45)"
    )
    ...
    ap.add_argument(
        ...
        default=60,
        dest="max_repos",
        help="stop after N repos (default 60)",
    )
    ap.add_argument(
        "--redact",
        action="store_true",
    ...
    if args.state and os.path.exists(args.state):
        try:
            ...
            print(
                "warning: could not read state file %s; starting fresh" % args.state,
                file=sys.stderr,
            )
    ...
    if args.mark_resurrected:
        if not args.state:
            print(
                "error: --mark-resurrected needs --state FILE to write into",
                file=sys.stderr,
            )
            return 2
        target = str(Path(os.path.expanduser(args.mark_resurrected)).resolve())
        state["resurrections"] = [
            r for r in state["resurrections"] if r["path"] != target
        ]
        ...
        print(
            "recorded: %s resurrected %s. The next scan holds it to that."
    ...
    if not paths:
        ...
        if unversioned:
            print(
                "But %d project folder(s) with no git at all: %s"
                % (len(unversioned), ", ".join(p.name for p in unversioned[:8]))
            )
            print(
                "No history means no autopsy — get them under git before they rot further."
            )
        print("Pass the directories where your projects actually live.")
        return 1
    if len(paths) > args.max_repos:
        print(
            "(found %d repos; reading the first %d — raise --max to widen)"
            % (len(paths), args.max_repos),
            file=sys.stderr,
        )
    ...
    if len(paths) > 15:
        print(
            "reading %d repos (a few seconds each on big histories)..." % len(paths),
            file=sys.stderr,
        )
    repos, skipped = [], []
    for p in paths:
        ...
        if not args.include_foreign and my_emails and r["mine"] / r["commits"] < 0.2:
            skipped.append(
                "%s (%d/%d commits yours)" % (r["name"], r["mine"], r["commits"])
            )
    ...
    print("=" * 60)
    print(
        "repos scanned: %d   alive: %d   finished: %d   dead: %d   not yours (skipped): %d"
        % (len(repos) + foreign, len(alive), len(finished), len(dead), foreign)
    )
    if finished:
        print(
            "finished (shipped, just stable — not corpses): %s"
            % ", ".join(r["name"] for r in finished[:8])
        )
    if skipped:
        print(
            "skipped as not-yours (by commit email): %s"
            % ", ".join(s.split(" (")[0] for s in skipped[:10])
            + (" …" if len(skipped) > 10 else "")
        )
        print(
            "  (yours under another email? rerun with --me that@email.com, or --include-foreign)"
        )
    if unversioned:
        if args.redact:
            print(
                "unversioned (no git — died before their first commit): %d project folder(s)"
                % len(unversioned)
            )
        else:
            print(
                "unversioned (no git — died before their first commit): %s"
    ...
    print(
        "oldest corpse: %s (silent since %s)"
    ...
    for r in dead:
        ...
        print(
            "%-30s %4dd %9d   %s"
            % (r["name"][:30], r["lifespan_days"], r["commits"], cause)
        )
    ...
    print(
        "- median lifespan of a dead project: %d day%s"
        % (med_span, "" if med_span == 1 else "s")
    )
    burst = [r for r in dead if r["lifespan_days"] <= 1 and r["commits"] >= 5]
    if len(burst) >= 2:
        print(
            "- %d projects lived exactly one day: built in a single burst, never reopened."
            % len(burst)
        )
    shiny = [r for r in dead if r["causes"][0][0] == "shiny_object"]
    if shiny:
        print(
            "- %d of %d were killed by a newer project. You don't abandon projects;"
            % (len(shiny), len(dead))
        )
    ...
    if len(walls) >= 2:
        print(
            "- %d projects died at the same wall (auth/payments). That wall isn't moving;"
            % len(walls)
        )
    ...
    if fear:
        print(
            "- %d finished project(s) never shipped. Building was never the problem."
            % len(fear)
        )
    ...
    if state["resurrections"]:
        ...
        for res in state["resurrections"]:
            ...
            if r is None:
                lines.append(
                    "- %s: resurrected %s, no longer found on disk. Buried for good, or moved."
                    % (res["name"], res["date"])
                )
                continue
            ...
            if silent_days > 14:
                lines.append(
                    "- %s: resurrected %s, silent for %d days since. It's dying again — "
                    "decide: recommit or bury it honestly."
                    % (res["name"], res["date"], silent_days)
                )
            else:
                lines.append(
                    "- %s: resurrected %s, last commit %dd ago. Holding."
                    % (res["name"], res["date"], silent_days)
                )
        print("\nRELAPSE WATCH")
        print("-" * 60)
            ...
            json.dump(
                {
                    ...
                    "alive": [
                        {k: r[k] for k in ("name", "path", "last", "commits")}
                        for r in alive
                    ],
                    "finished": [
                        {k: r[k] for k in ("name", "path", "last", "commits")}
    ...
    if args.state:
        ...
        state["last_scan"] = {
            ...
            "dead": [
                {
                    "name": r.get("real_name", r["name"]),
                    "path": r.get("real_path", r["path"]),
                    "cause": r["causes"][0][0],
                    "pulse": r["pulse"],
                }
                for r in dead
