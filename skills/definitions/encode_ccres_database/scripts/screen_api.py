...
def cmd_search(args):
    ...
    if args.chromosome and args.start and args.end:
        coords = [{"chromosome": args.chromosome, "start": args.start, "end": args.end}]
...
def cmd_linked(args):
    ...
    """
    res, asm, err = run_query(query, {"accessions": args.accessions}, args.assembly)
    if err:
        print(f"Error: {err}", file=sys.stderr)
        sys.exit(1)
    write_output(res, asm, args.output)
...
def cmd_gwas(args):
    ...
    if args.type == "studies":
        ...
        """
        res, asm, err = run_query(query, {"study": args.study}, requires_assembly=False)
...
def main():
    ...
    p_search.add_argument("--accessions", nargs="+", help="One or more accessions")
    p_search.add_argument(
        "--ctcf-start", type=float, help="CTCF max z-score range start"
    )
    p_search.add_argument("--ctcf-end", type=float, help="CTCF max z-score range end")
    p_search.add_argument(
        "--cellType",
    ...
    p_details.add_argument("accession", help="cCRE accession (e.g., EH38E2941922)")
    ...
    add_common(p_linked)
    p_linked.add_argument("accessions", nargs="+", help="One or more cCRE accessions")
    ...
    p_gene_expr.add_argument("gene", help="Gene symbol (e.g., OR51B4, GAPDH, TP53)")
    ...
    p_gwas.add_argument("--study", help="Study name (required for snps and enrichment)")
    p_gwas.add_argument(
        "--output",
        default="/tmp/encode_output.json",
        help="Output JSON file path (default: /tmp/encode_output.json).",
