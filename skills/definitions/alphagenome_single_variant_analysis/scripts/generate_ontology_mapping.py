...
def clean_list(series: pd.Series) -> list[str]:
    ...
    vals = {
        v for v in vals if v.strip() and v.lower() not in ("none", "nan", "null", "")
    }
...
def prune_empty(d):
    ...
    if isinstance(d, list):
        return [v for v in (prune_empty(v) for v in d) if v not in (None, [], {}, "")]
...
def create_biological_mapping(df: pd.DataFrame) -> dict:
    ...
    df_proc["output_type"] = (
        df_proc["output_type"].astype(str).str.replace("OutputType.", "", regex=False)
    )
    mapping = {}
