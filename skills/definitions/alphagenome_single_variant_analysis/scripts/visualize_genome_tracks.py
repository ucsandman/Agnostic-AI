...
def filter_tracks_by_ontology(track: object, ontology_curie: str) -> object | None:
...
def add_track_component(
    ...
    if separate_strands and "strand" in filt.metadata.columns:
        ...
        components.append(plot_components.Tracks(filt, ylabel_template=f"{track_type}"))
...
def render_broad_view(
    ...
    if hasattr(ref_preds, "contact_maps") and ref_preds.contact_maps:
        if hasattr(plot_components, "ContactMaps"):
            ...
            print(
                "WARNING: plot_components.ContactMaps not found. Skipping Contact Maps."
            )
...
def render_zoom_view(
    ...
    if zoom_predictions.splice_junctions:
        j_filt = filter_tracks_by_ontology(zoom_predictions.splice_junctions, ontology)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--zoom_genes", help="Comma-separated genes to zoom into.")
