...
def analyze_splicing(
    ...
    print("\n--- Junction Analysis ---")
    junc_ref = prediction.reference.splice_junctions.filter_by_ontology(ontology_id)
    junc_alt = prediction.alternate.splice_junctions.filter_by_ontology(ontology_id)
    ...
    if not df_ref.empty and not df_alt.empty:
        ...
        variant_overlapping = merged[(merged["start"] < pos) & (merged["end"] > pos)]
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument("--chrom", required=True, help="Chromosome (e.g., chr21).")
