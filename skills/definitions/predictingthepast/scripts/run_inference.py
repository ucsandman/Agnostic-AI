...
_MODELS_DIR: str = os.path.join(os.path.expanduser("~"), ".predictingthepast", "models")
...
def main() -> None:
    ...
    windows = split_into_windows(text, max_len=_MAX_LEN, overlap=args.window_overlap)
    print(
    ...
    if args.contextualize:
        if not region_names_list:
            region_names_list = _load_region_names(args.language)
        final_ctx = [_enrich_contextualization(c, region_names_list) for c in all_ctx]
    else:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run Predicting The Past inference.")
    parser.add_argument(
        "--language",
        required=True,
        choices=["latin", "greek"],
    ...
    parser.add_argument("--output_json", default="", help="Save combined JSON to file.")
