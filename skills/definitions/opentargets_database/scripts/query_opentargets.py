        ...
        for k, v in data.items():
            if k == "rows" and isinstance(v, list) and "count" in data:
                ...
                if len(v) > limit:
                    ...
                    result[k].append(
                        {"_truncated": f"(Showing {limit} of {count} rows)"}
                    )
                else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=200,
        help=(
            "API pagination size (default: 200). Set higher if results are truncated."
        ),
    )
    parser.add_argument(
        "--output",
    ...
    p_l2g = subparsers.add_parser(
        "get-l2g",
        help=(
            "Get L2G prioritisation scores for a variant, optionally filtered by study"
        ),
    )
    p_l2g.add_argument(
        "variant_id",
    ...
    p_drug.add_argument("ensembl_id", help="Target Ensembl ID (e.g., ENSG00000169083)")
    ...
    p_near.add_argument("ensembl_id", help="Target Ensembl ID (e.g., ENSG00000156515)")
    p_near.add_argument(
        "--window",
        type=int,
        default=500_000,
    ...
    p_custom.add_argument("--variables", help="JSON string of variables", default="{}")
    ...
    if args.command == "get-gwas-studies":
        ...
        for row in rows:
            ...
            v_chrom = row.get("chromosome") or row.get("variant", {}).get("chromosome")
            if (
                v_pos is not None
