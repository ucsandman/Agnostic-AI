...
def _normalise_rsid(raw):
    ...
    if not text.isdigit():
        _die(f"Invalid rsID '{raw}'. Provide a numeric ID such as '268' or 'rs268'.")
    return text
...
def _resolve_hgvs_for_assembly(hgvs, assembly):
    ...
    url = f"{_BASE_URL}/variation/v0/hgvs/{encoded}/contextuals?assembly={assembly}"
    try:
        resp = _fetch_json(url, allow_not_found=True, allow_ref_mismatch=True)
    except ReferenceMismatchError:
        return []
...
def main():
    ...
    p_hgvs.add_argument("hgvs", help="HGVS string (e.g. NC_000008.11:g.19962213del).")
    p_hgvs.add_argument(
        "--assembly",
        default="GCF_000001405.40",
        help="RefSeq assembly accession (default: GCF_000001405.40).",
    ...
    p_region.add_argument("--output", required=True, help="Output JSON file path.")
