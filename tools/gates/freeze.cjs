...
// realpath first: ~/.claude/hooks is a junction into the repository, and a frozen file is the same
// file whichever door it is reached through.
const real = (p) => { try { return fs.realpathSync.native(p); } catch (_) { return p; } };
const norm = (p) => real(path.resolve(p)).replace(/\\/g, '/').replace(/^([a-z]):/, (_, d) => d.toUpperCase() + ':');
const expand = (p) => norm(p.replace(/^~(?=\/|$)/, HOME).replace(/^(?![A-Za-z]:|\/)/, ROOT + '/'));

// The overlay adds machine-specific roots and frozen files (a product repo whose hooks this machine
// also freezes); it never removes anything the repository freezes.
const OVERLAY = path.join(HOME, '.claude', 'overlay', 'gates.json');
function manifest() {
  const m = JSON.parse(fs.readFileSync(MANIFEST, 'utf8'));
  let o = {};
  try { o = JSON.parse(fs.readFileSync(OVERLAY, 'utf8')); } catch (_) { /* no overlay */ }
  return { harnessRoots: [...(m.harnessRoots || []), ...(o.harnessRoots || [])].map(expand), frozen: [...(m.frozen || []), ...(o.frozen || [])] };
}
