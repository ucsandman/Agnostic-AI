---
name: hook-latency-2026-09-06
description: "The 2026-09-06 pass: two hooks ported from pwsh and python to node, a dead rewrite removed, the DashClaw round trip collapsed, four hooks made async"
context:
  triggers:
    keywords: ["correction-tracker", "session-count", "dashclaw round trip", "claim_execution", "async hook"]
  requires: ["hook-latency"]
  priority: 30
---
# Hook latency pass of 2026-09-06

Every registered hook was timed three times with a real payload
(`labs/tokflow/` has the earlier token audit; the latency table is in memory
`harness-latency-2026-09-06`). Hooks for one event run in parallel, so an
...
- Not changed, with numbers: the 13 node guards on a Bash call cost ~175 ms
  ...
  (On a slow machine this inverts; see the 2026-09-19 pass.)

## DashClaw round trip collapsed, four hooks async (later the same day)
