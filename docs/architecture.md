...
| `fable-delegate-guard.cjs` | Delegation briefing and hand-work log for a Fable main loop (Claude Code only). Detects the session model from the transcript tail, injects the token-economics briefing once per session, and logs large edits and code-writing shell commands outside `~/.claude` and the scratchpad for `--report`. Denies nothing since 2026-09-06 (the block produced 714 overrides and retried denials; see CHANGELOG). Subagents are exempt via `agent_id`. Owns the shell classifier the Codex twin imports. |
| `capability-graph-guard.cjs` | Enforces the subagent capability graph in Claude Code (Fable -> Opus/Sonnet/Haiku, Opus -> Sonnet/Haiku, Sonnet -> Haiku, Haiku -> nobody). Resolves each subagent's model lazily (`callerModelFor`: per-session registry -> `subagent_config` -> the `agent-<id>.meta.json` sidecar Claude Code writes under the session dir -> the subagent's own transcript -> agent-file frontmatter) and caches it, then denies any Agent/Task/Workflow call that is upward or peer. The one upward edge is `subagent_type: advisor` (read-only Fable guidance, 2 per agent and 3 per session). Kill switch: `CAPABILITY_GRAPH_GUARD=off`. |
