...
| `fable-delegate-guard.cjs` | Delegate-first enforcement for a Fable main loop (Claude Code only). Detects the session model from the transcript tail, then budgets direct edits (free under `~/.claude` and the scratchpad, 3 small ones per prompt elsewhere) and denies code-writing shell commands. Subagents are exempt via `agent_id`. Also injects the delegation briefing once per session. |
