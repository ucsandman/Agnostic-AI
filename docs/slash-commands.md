...
| `/org on\|off\|status\|tree\|config\|prune\|mode` | both | Control and inspect adaptive orchestration. `/org prune` removes orphaned isolated worktrees and reports the count. `/org mode auto\|hierarchy\|advisor` changes routing emphasis without weakening role, depth, fanout, workspace, or advisor limits. |
