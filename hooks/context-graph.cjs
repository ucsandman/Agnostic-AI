...
//
// Engine (client-neutral, tested): C:/Projects/agnostic-ai/engine/context/.
...
const CONFIG = process.env.CONTEXT_GRAPH_CONFIG || path.join(__dirname, 'context-graph.json');
...
function loadEngine(cfg) {
  const p = process.env.CONTEXT_GRAPH_ENGINE || cfg.engine || 'C:/Projects/agnostic-ai/engine/context/index.cjs';
...
function readConfig() {
  const { engine } = loadEngine(JSON.parse(fs.readFileSync(CONFIG, 'utf8')));
  return engine.config.load(CONFIG);
}
function engineFile() { return process.env.CONTEXT_GRAPH_ENGINE || JSON.parse(fs.readFileSync(CONFIG, 'utf8')).engine; }
